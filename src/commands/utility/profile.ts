import { ActionRowBuilder, ButtonBuilder, ButtonInteraction, ButtonStyle, ComponentType, EmbedBuilder, GuildMember, SlashCommandBuilder } from "discord.js";
...
export const profile: Command = {
    ...
    execute: async (context: CommandContext, t: TFunction, args: ParsedArgs) => {
        ...
        const infoEmbed = new EmbedBuilder()
            ...
            .setColor(0x5865F2)
            .addFields(
                { name: t('commands:profile.fields.name'), value: `\`${target.displayName}\``, inline: true },
        ...
        const statsEmbed = new EmbedBuilder()
            ...
            .setColor(0xFEE75C)
            .addFields(
                { name: t('commands:profile.fields.level'), value: `**Lv.${progress.level}**`, inline: true },
                { name: t('commands:profile.fields.rank'), value: `**#${profileData?.global_rank || 'N/A'}**`, inline: true },
                { name: t('commands:profile.fields.total_xp'), value: `\`${xp.toLocaleString()}\``, inline: true },
                { 
        ...
        const message = 'author' in context
            ? (response as any)
            : await (context as any).fetchReply();
